...
routes.get("me:dashboard","/dashboard",{permission:"me:dashboard:read"}).handle(async (req,res,next) => {
  ...
  dashboard.program = await Event.find({ dateTill: { $gte: new Date() }, status: { $in: ["public","cancelled"] } }).select("_id name dateFrom dateTill leaders").populate("leaders","nickname").limit(5).toObject();
