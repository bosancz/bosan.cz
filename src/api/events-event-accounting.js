...
routes.post("event:accounting","/",{permission:"events:edit"}).handle(upload.single("file"), async (req,res,next) => {
...
routes.get("event:accounting-template","/template",{permission:"events:read"}).handle(async (req,res,next) => {
  ...
  const leadersString = event.leaders[0] && event.leaders[0].name ? event.leaders[0].name.first + " " + event.leaders[0].name.last : "";
  ...
  const expenses = event.expenses.map(expense => [ expense.id, expense.description, expense.amount ]);
