...
import connectionMongo from "../db";
...
async function importEvents() {
  ...
  var groups = await new Promise((resolve, reject) =>
    connectionSql.query("SELECT * FROM bo_akce_oddilu", (err, result, fields) => (err ? reject(err) : resolve(result)))
  );
  var events = await new Promise((resolve, reject) =>
    connectionSql.query(
      "SELECT PA.*,TA.typ FROM bo_program_akci AS PA LEFT JOIN bo_typ_akce AS TA ON TA.id_typ = PA.typ_akce_id",
