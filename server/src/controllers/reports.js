...
routes.get("reports:events", "/events/:year", { permission: "reports:events:read" }).handle(async (req, res, next) => {
  const query = Event.find({ status: "public" }, null, { autopopulate: false }).select(
    "_id name dateFrom dateTill groups leaders attendees"
  );
  ...
  query.where({
    dateFrom: {
      $gte: DateTime.local()
        .set({ day: 1, month: 1, year: Number(req.params.year) })
        .toJSDate(),
      $lt: DateTime.local()
        .set({ day: 1, month: 1, year: Number(req.params.year) + 1 })
        .toJSDate(),
    },
  });
  ...
  report.events.top = events
    ...
    .map((event) => ({
      ...
      leaders: event.leaders.map((leader) => ({ nickname: leader.nickname })),
  ...
  report.leaders = events
    .map((event) => {
      return event.leaders.map((member) => ({
        ...
        event: event,
        age: member.birthday
          ? Math.min(26, Math.max(7, Math.floor(-1 * DateTime.fromJSDate(member.birthday).diffNow("years").years)))
          : undefined,
    ...
    .reduce(
      (acc, cur) => {
        cur.forEach((leader) => {
          ...
          if (!acc.top[leader.member._id])
            acc.top[leader.member._id] = {
              member: { _id: leader.member._id, nickname: leader.member.nickname },
              events: [],
            };
  ...
  report.leaders.top = Object.values(report.leaders.top)
    ...
    .reduce(
      (acc, cur) => {
        cur.forEach((attendee) => {
          ...
          if (attendee.group)
            acc.groups[attendee.group] = acc.groups[attendee.group] ? acc.groups[attendee.group] + 1 : 1;
...
routes.get("reports:members", "/members", { permission: "reports:members:read" }).handle(async (req, res, next) => {
  const members = await Member.find({ inactive: { $ne: true } })
    .select("_id role birthday address.city")
    .lean();

  const report = {
    count: members.length,

    roles: members.reduce((acc, cur) => {
      acc[cur.role] = acc[cur.role] ? acc[cur.role] + 1 : 1;
      return acc;
    }, {}),
    ...
    ages: members
      .map((member) => ({
        age: member.birthday
          ? String(Math.floor(-1 * DateTime.fromJSDate(member.birthday).diffNow("years").years))
          : undefined,
        role: member.role,
      }))
