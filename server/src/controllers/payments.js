  ...
  properties: {
    filter: {
      ...
      properties: {
        timestamp: {
          type: "object",
          properties: { $gte: { type: "string", format: "date" }, $lte: { type: "string", format: "date" } },
...
routes
  .get("payments", "/", { permission: "payments:list" })
  .handle(validate({ query: getPaymentsSchema }), async (req, res) => {
...
routes
  .get("payment", "/:id", { permission: "payments:read" })
  .handle(async (req, res) => res.json(await Payment.findOne({ _id: req.params.id })));
