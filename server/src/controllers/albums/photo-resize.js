...
export default async function (file, sizes, options) {
  ...
  /* PHOTO STATISTICS */
  if (stats) {
    var metadata = await Promise.all([sharp(file).metadata(), sharp(file).stats()]);
    ...
    result.stats = {
      date:
        (origExif && origExif.exif && origExif.exif.DateTimeOriginal) ||
        (origExif && origExif.image && origExif.image.ModifyDate),
      bg:
        "rgb(" +
        metadata[1].channels
          .slice(0, 3)
