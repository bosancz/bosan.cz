...
routes
  .post("event:announcement", "/", { permission: "events:edit" })
...
routes.get("event:announcement-template", "/template", { permission: "events:read" }).handle(async (req, res, next) => {
  ...
  const attendees = attendeeMembers.map((member) => [
    ...
    (member.contacts && member.contacts.mobile) ||
      (member.contacts.mother ? "Matka: " + member.contacts.mother : null) ||
      (member.contacts.father ? "Matka: " + member.contacts.father : null) ||
      missing,
  ]);
