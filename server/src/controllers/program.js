...
routes.get("program:ical", "/ical", { permission: "program:read" }).handle(async (req, res, next) => {
  ...
  query.populate("leaders", "_id nickname name.first contacts.email");

  const events = await query;

  for (let event of events) {
    const leader =
      (event.leaders[0].nickname || event.leaders[0].name?.first) && event.leaders[0].contacts?.email
        ? event.leaders[0]
        : undefined;

    const organizer = leader
      ? `${leader.nickname || leader.name?.first} <${leader.contacts?.email}>`
      : config.ical.organizer;

    const attendees = event.leaders
      .slice(leader ? 1 : 0)
      .filter((member) => (member.nickname || member.name?.first) && member.contacts && member.contacts.email)
      .map((member) => ({ name: member.nickname || member.name?.first, email: member.contacts.email, rsvp: true }));
    ...
    cal.createEvent({
      ...
      allDay: true,
      summary: `${event.name} (${event.groups.join(", ")})`,
