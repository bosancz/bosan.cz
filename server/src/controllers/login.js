...
import saveCookie from "./login/save-cookie";
import clearCookie from "./login/clear-cookie";
...
routes
  .post("login", "/", { permission: "login:credentials" })
  .handle(validate({ body: loginSchema }), async (req, res, next) => {
    if (!req.body.login) return res.status(400).send("Missing login");
    if (!req.body.password) return res.status(400).send("Missing password");

    let login = req.body.login.toLowerCase();

    var user = await User.findOne({ $or: [{ login: login }, { email: login }] })
      .select("+password")
      .lean();
    ...
    if (bcrypt.getRounds(user.password) < config.auth.bcrypt.rounds) {
      ...
      bcrypt
        .hash(req.body.password, config.auth.bcrypt.rounds)
        .then((hash) => {
          user.password = hash;
          user.save();
        })
...
routes
  .post("login:sendlink", "/sendlink", { permission: "login:link" })
  .handle(validate({ body: sendLinkSchema }), async (req, res) => {
    ...
    const code = await axios
      .get(
        "https://www.random.org/strings/?num=2&len=16&digits=on&upperalpha=on&loweralpha=on&unique=on&format=plain&rnd=new",
        { responseTYpe: "text" }
      )
